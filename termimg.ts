...
export function detectProtocol(): Protocol {
  ...
  if (env.KITTY_WINDOW_ID || term.includes("xterm-kitty") || env.WEZTERM_EXECUTABLE) {
    return "kitty";
  }
  ...
  if (process.stdout.isTTY && (env.COLORTERM?.includes("truecolor") || env.COLORTERM === "24bit")) {
    return "ansi";
  }
  return "none";
}
...
export function iterm2SequenceFromFile(
  filePath: string,
  { width = "auto", height = "auto", preserveAspectRatio = true }: RenderOpts = {}
): string {
 ...
 */
function kittySerialize(params: Record<string, string | number>, payloadB64: string) {
  const keys = Object.entries(params).map(([k, v]) => `${k}=${v}`).join(",");
...
export async function ansiHalfBlockFromImage(
  ...
  img.resize({ w: cellCols, h: cellRows * 2, mode: Jimp.ResizeStrategy.BILINEAR });
  ...
  for (let y = 0; y < cellRows; y++) {
    ...
    for (let x = 0; x < cellCols; x++) {
      ...
      line += `${ESC}[38;2;${top.r};${top.g};${top.b}m` + // fg
...
export async function getImageSequence(tmpFile: string, size = 100) {
    const protocol = process.env.TERM_PROGRAM === 'ghostty' ? 'kitty' : detectProtocol();
    ...
    try {
      switch (protocol) {
        ...
        case "iterm2":
          ...
          return itermSeq;
        case "ansi":
          const ansiSeq = await ansiHalfBlockFromImage(tmpFile, cols);
